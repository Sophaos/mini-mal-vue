                ...
                <div class="flex justify-content-between flex-wrap w-full">
                  ...
                  <div class="flex align-items-center justify-content-center">
                    <div v-for="tag of review.tags" :key="tag">
                      <span v-if="tag === 'Recommended'" class="text-green-500">{{
                        tag
                      }}</span>
                      <span v-if="tag === 'Mixed Feelings'" class="text-yellow-500">{{
                        tag
                      }}</span>
                      <span v-if="tag === 'Not Recommended'" class="text-red-600">{{
                        tag
                      }}</span>
                    </div>
                  </div>
                </div>
                <p class="text-sm max-h-7rem w-auto overflow-auto">{{
                  review.content
                }}</p>
